...
export class SessionDocumentRepository implements SessionRepository {
  ...
  async findOne(fields: EntityCondition<Session>): Promise<NullableType<Session>> {
    if (fields.id) {
  ...
  async update(id: Session["id"], payload: Partial<Session>): Promise<Session | null> {
    ...
    const filter = { _id: id };
    const sessionObject = await this.sessionModel.findOneAndUpdate(filter, clonedPayload);
