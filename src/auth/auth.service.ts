...
export class AuthService {
  ...
  async register(dto: AuthRegisterLoginDto): Promise<void> {
    ...
    const hash = await this.jwtService.signAsync(
      ...
      {
        secret: this.configService.get<string>("auth.confirmEmailSecret", {
          infer: true,
        }),
        expiresIn: this.configService.get<string>("auth.confirmEmailExpires", {
          infer: true,
        }),
      },
    );
  ...
  async forgotPassword(email: string): Promise<void> {
    ...
    const tokenExpiresIn = this.configService.get<string>("auth.forgotExpires", {
      infer: true,
    });
    ...
    const hash = await this.jwtService.signAsync(
      ...
      {
        secret: this.configService.get<string>("auth.forgotSecret", {
          infer: true,
        }),
  ...
  async refreshToken(data: Pick<JwtRefreshPayloadType, "sessionId" | "hash">): Promise<Omit<LoginResponseType, "user">> {
  ...
  private async getTokensData(data: { id: User["id"]; role: User["role"]; sessionId: Session["id"]; hash: Session["hash"] }) {
    const tokenExpiresIn = this.configService.get<string>("auth", {
      infer: true,
    });
